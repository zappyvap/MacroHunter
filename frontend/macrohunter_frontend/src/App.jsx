...
const mockResults = (cals, protein, carbs, fats) => [
  {
    ...
    dish: "Double Protein Bowl",
    distance: "0.3 mi",
    type: "American Grill",
    cal: Math.round(cals * 0.95),
    protein: Math.round(protein * 1.05),
    carbs: Math.round(carbs * 0.88),
    fats: Math.round(fats * 0.92),
    score: 96,
    topPick: true,
  },
  {
    ...
    distance: "0.7 mi",
    type: "Meal Prep",
    cal: Math.round(cals * 1.02),
    protein: Math.round(protein * 0.98),
    carbs: Math.round(carbs * 1.03),
    fats: Math.round(fats * 1.1),
    score: 89,
    topPick: false,
  },
  {
    id: 3,
    name: "Zen Noodle Bar",
    dish: "High-Protein Ramen",
    distance: "1.1 mi",
    type: "Japanese",
    cal: Math.round(cals * 0.88),
  ...
  },
  {
    id: 4,
    name: "Verde Fresh Co.",
    dish: "Power Grain Bowl",
    distance: "1.4 mi",
    type: "Health Cafe",
    cal: Math.round(cals * 0.82),
    protein: Math.round(protein * 0.85),
    carbs: Math.round(carbs * 0.9),
    fats: Math.round(fats * 0.8),
    score: 68,
    topPick: false,
  },
];
          ...
          <div className="panel-right">
            <div className="results-header">
              <div className="results-title">
                {results ? "Nearby Matches" : loading ? "Scanning..." : "Results"}
              </div>
              {results && (
                <div className="results-count">{results.length} locations found</div>
              )}
            </div>
            ...
            {!loading && results && (
              <div className="results-list">
                {results.map((r, i) => (
                  ...
                  >
                    {r.topPick && <div className="top-badge">⚡ Top Pick</div>}
                    <div>
                    ...
                    <div className="result-right">
                      <div>
                        <div className="result-cal">{r.cal}</div>
            ...
            {!loading && !results && (
              <div className="empty-state">
                ...
                <div className="empty-sub">
                  Set your macro targets and calories on the left, then hit the search button to find meals near you.
                </div>
              </div>
            )}
          </div>
